...
from utils import get_noise_schedule_list
from .blocks import (
    Embedding,
...
class GaussianDiffusion(nn.Module):
    def __init__(self,
        ...
        self.denoise_fn = Denoiser(n_mel_channels, encoder_hidden, residual_channels, residual_layers, denoiser_dropout)
        ...
        with open(
            ...
            stats = json.load(f)
            self.register_buffer("spec_min", torch.FloatTensor(stats["spec_min"]))
            self.register_buffer("spec_max", torch.FloatTensor(stats["spec_max"]))
